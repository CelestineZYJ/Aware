...
class PhiModel(PhiPreTrainedModel):
    ...
    def __init__(self, config: PhiConfig):
        ...
        # self.fc_time = torch.nn.Linear(1, 1)
        self.hidden_time_encoder = Hidden_Time_Encoder()
        self.hidden_time_decoder = Hidden_Time_Decoder()
    ...
    def forward(
        ...
        attention_mask: Optional[torch.Tensor] = None,
        time_ids: torch.LongTensor = None,
        position_ids: Optional[torch.LongTensor] = None,
        ...
        if self.gradient_checkpointing and self.training:
            ...
            layer_outputs = first_decoder_layer(
                hidden_states,
                attention_mask=attention_mask,
                position_ids=position_ids,
...
class Hidden_Time_Encoder(VAEBaseModel):
    ...
    def __init__(self):
        super(Hidden_Time_Encoder, self).__init__()

        # input: Batch_size * 3 * 256 * 256
        self.layer_dims = [1024, 512, 256, 218]
...
class Hidden_Time_Decoder(VAEBaseModel):
